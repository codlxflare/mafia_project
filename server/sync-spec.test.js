...
describe('Спецификация синхронности (порядок в коде)', () => {
  ...
  it('день: после night_summary идёт delay, затем day_discussion, затем delay, затем phase day', () => {
    const summaryIdx = serverCode.indexOf("getHostLine('night_summary'");
    assert(summaryIdx >= 0);
    const after = serverCode.slice(summaryIdx);
    assert(after.includes("getHostLine('day_discussion'"), 'После итога ночи — реплика day_discussion');
