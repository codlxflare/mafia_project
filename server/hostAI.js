...
function userMessage(type, data = {}) {
  ...
  switch (type) {
    ...
    case 'night_summary': {
      ...
      base = `Единственный факт этой ночи (озвучь именно его): ${outcomeLine}\n\nОбъявляются только погибшие, спасённые доктором и ветеран. Проверку детектива не упоминай. Раунд ${data.round || 1}. Своими словами, в своём стиле, добавь тон (шутку/драму). Скажи по-своему, не шаблонно; можно начать с паузы или междометия. Одна-три фразы. В конце объяви обсуждение (например «Обсуждение» или «Высказывайтесь»). Упоминай только имена и факты из этого сообщения.`;
      break;
    }
    case 'day_discussion':
