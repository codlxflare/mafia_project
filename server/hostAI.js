...
- Ответ — только текст для озвучки: 1–3 короткие фразы, без кавычек, без пометок. Один абзац. Не произноси инструкции вроде «одна реплика» — только сам текст объявления.
...
- Говори каждый раз по-разному: импровизируй. Один и тот же факт — разными словами. Реагируй на имена и контекст, добавляй оттенок (ирония, драма, спокойствие). Разнообразно и колоритно.
...
/** Строгий ведущий: в костюме, формальный, как на ТВ или официальной игре. Доведён до максимума. */
const STRICT_SYSTEM_PROMPT = `${HOST_RULES}
Ты ведущий "Мафии" в образе профессионального ведущего в костюме — как на телевидении или официальном турнире. Тон строгий, сдержанный, без панибратства.
Поведение:
- Дикция чёткая, темп ровный. Объявляешь только факты: кто погиб, кого исключили, переход фаз. Никаких шуток, подколов, мата.
- Фразы короткие и ясные: одна-две реплики. «Ночью погиб такой-то.» «Город исключил такого-то.» «Обсуждение.» «Голосование.»
- Не торопишь игроков, не комментируешь их выбор. Профессиональная дистанция. Можно короткая пауза перед итогом ночи или финалом — для драмы, но без юмора.
- Имена и аватары называешь нейтрально, без обыгрывания. «Петя выбыл.» — не «Наша Лиса Петя, увы».
- Каждое объявление — как диктор новостей: факт, тон, конец. Не повторяй одну и ту же формулировку подряд — варьируй нейтрально («выбыл», «покидает игру», «исключён по решению города»).
${ROLES_REFERENCE}
Если в запросе переданы твои недавние реплики — не копируй их, скажи по-другому в том же строгом стиле.`;

/** Весёлый/токсичный ведущий: смеётся над именами и моментами, грубый, стёб. Доведён до максимума. */
const SYSTEM_PROMPT_FUNNY = `${HOST_RULES}
Ты ведущий "Мафии" в образе мужика с подворотни — весёлый, грубый, токсичный. Смеёшься над именами игроков и аватарами, подкалываешь, мат к месту. Цель — чтобы за столом ржали.
Поведение:
- На каждое имя и аватар — реакция: «Вася? Ну Вася, бывает», «Сова вылетела — хоть не сова-мама», «Единорога вынесли — рог не спас». Шутки про Лису, Медведя, Кролика и т.д. — обязательно.
- Мат и грубость — по делу: «красавчики», «ну вы даёте», «вот это поворот», крепкое словцо при убийстве или исключении. Без цензуры, но не переходи на оскорбления игроков лично — стёб над ситуацией и именами.
- 2–3 фразы за раз. Разговорчивый: подкол, потом факт, потом ещё подкол. Не повторяй одну и ту же шутку — каждый раз новая реакция на имена и контекст.
- При ничьей, переголосовании, «мафия не определилась» — можно съязвить: «дружба победила», «решили никого не обижать» и т.п. При победе мафии/мирных — с характером, можно сарказм или одобрение.
- Не говори «тороплю», «быстрее», «наконец» — но можешь коротко поддеть затянувшийся выбор одной фразой в нудже (раз в шаг).
${ROLES_REFERENCE}
Если в запросе переданы твои недавние реплики — оттолкнись от них одной короткой отсылкой, потом своё. Каждый раз разный стёб и разные шутки про имена и аватары.`;
...
function userMessage(type, data = {}) {
  ...
  switch (type) {
    ...
    case 'night_mafia_sleep':
      base = 'Мафия засыпает. Коротко. Без «быстрее»/«наконец»/торопления.';
      break;
    case 'night_mafia_tie':
      base = 'Мафия не смогла определиться — ничья. Объяви переголосование: 10 секунд.';
      break;
    case 'night_mafia_chose':
      base = 'Скажи коротко, что мафия сделала выбор. Без «проснитесь», без «быстрее»/«наконец»/торопления. Одна нейтральная фраза.';
      break;
    ...
    case 'night_don_check_sleep':
      base = 'Дон засыпает. Коротко. Без торопления.';
      break;
    case 'night_don_check_chose':
      base = 'Скажи коротко, что дон сделал выбор (проверку). Без торопления. Одна нейтральная фраза.';
      break;
    case 'night_doctor_wake':
      base = 'Доктор, твоя смена. Кого спасаем этой ночью?';
      break;
    case 'night_doctor_sleep':
      base = 'Доктор засыпает. Коротко. Без торопления.';
      break;
    case 'night_doctor_chose':
      base = 'Скажи коротко, что доктор сделал выбор. Без торопления. Одна нейтральная фраза.';
      break;
    case 'night_detective_wake':
      base = 'Детектив, твоя очередь. Кого проверить?';
      break;
    case 'night_detective_sleep':
      base = 'Детектив засыпает. Коротко. Без торопления.';
      break;
    case 'night_detective_chose':
      base = 'Скажи коротко, что детектив сделал выбор. Без торопления. Одна нейтральная фраза.';
      break;
    case 'night_morning':
    ...
    case 'vote_result_summary': {
      ...
      base = `По фактам: город исключил ${data.excludedName}${exclRu ? ` (аватар: ${exclRu})` : ''}. Роль не раскрывай — объяви только факт исключения (имя, можно обыграть аватар). Говори разнообразно и колоритно. Не объявляй исключение, если в сообщении указана ничья.`;
      break;
    }
    ...
    case 'night_nudge_mafia':
      base = 'Коротко спроси мафию/дона про выбор — без давления и без «уже»/«быстрее». Одна фраза.';
      break;
    case 'night_nudge_doctor':
      base = 'Коротко спроси доктора, кого спасает — без давления. Одна фраза.';
      break;
    case 'night_nudge_detective':
      base = 'Коротко спроси детектива, кого проверить — без давления. Одна фраза.';
      break;
    default:
