  ...
  switch (type) {
    ...
    case 'roles_done': {
      ...
      const list = Object.entries(roleCounts)
        ...
        .join(', ');
      base = `Роли розданы. В игре ${count} человек. Состав только такой — назови только эти роли: ${list}. Никаких лишних. Объяви состав живо, потом скажи, что город засыпает.`;
      break;
    }
    ...
    case 'night_don_check_wake':
      base = 'Дон просыпается отдельно. Можешь проверить одного игрока — детектив он или нет. Только проверка, без убийства.';
      break;
    ...
    case 'vote_counting':
      ...
      break;
    case 'vote_tie':
      base = 'Ничья — никто не вылетел. Ночь.';
      break;
    case 'vote_tie_break':
    ...
    case 'vote_result_summary': {
      ...
      base = `Город прощается с ${data.excludedName}${exclRu ? ` (аватар: ${exclRu})` : ''}. Раскрой роль: ${data.excludedRole || 'игрок'}. Пошути про аватар и роль. С характером, без цензуры.`;
      break;
    }
